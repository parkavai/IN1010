...
class Lenkeliste<T> implements Liste<T>{
    ...
    private void leggTilRek(Node n, T element){
        if (start == null) {
            ...
            Node nye = new Node(element);
            n.neste = nye;
        }
        else {
            leggTilRek(n.neste, element);
        }
    }
...
/*
public int storrelseRek(){
    return storrelseRek(start, 0);
}

public int storrelseRek(Node n, int antallSaaLangt){
    // Basistilfellet
    if (n == null){
        return antallSaaLangt;
    }
    // rekursjonssteg
    return storrelseRek(n.neste, antallSaaLangt+1);
}
*/